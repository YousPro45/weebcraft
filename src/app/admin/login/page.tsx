...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: { preventDefault(): void }) => {
    ...
    const { error: err } = await supabase.current.auth.signInWithPassword({ email, password });
    if (err) { setError("Email ou mot de passe incorrect."); setLoading(false); }
    else router.push("/admin");
  };

  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg,#0D1F3C 0%,#1A3A6B 100%)" }}>
      <motion.div initial={{ opacity:0, y:40 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6 }} className="w-full max-w-md px-4">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-5">
            <div>
              ...
              <div className="relative">
                <Mail size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5A738A]" />
                <input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@webcraft.ma"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={()=>setShowPwd(!showPwd)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5A738A] hover:text-[#1E90D4]">
                  {showPwd ? <EyeOff size={17}/> : <Eye size={17}/>}
                </button>
              </div>
            </div>
            {error && <div className="text-red-600 text-sm bg-red-50 rounded-xl px-4 py-3 border border-red-100">{error}</div>}
            <button type="submit" disabled={loading} className="btn-primary justify-center py-3.5 text-base disabled:opacity-60">
